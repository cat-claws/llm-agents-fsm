...
# FSM/planning behavior is implemented as bindable functions below.
...
def _snapshot_json(self, snapshot) -> str:
    return json.dumps(snapshot, indent=2, sort_keys=True)

def _json_or_none(self, value) -> str:
    if value in (None, [], {}):
        return 'None'
    return json.dumps(value, indent=2, sort_keys=True)

def _recent_actions(self, accepted_trace: List[Dict[str, object]], limit: int = 6) -> List[str]:
    names = []
...
def _action_signature(self, action: Dict[str, object]) -> str:
    return json.dumps({
...
def _recent_action_signatures(
    ...
    for action in accepted_trace[-limit:]:
        if isinstance(action, dict):
            signatures.append(self._action_signature(action))
    return signatures

def _identical_repeat_warning(self, accepted_trace: List[Dict[str, object]]) -> str:
    signatures = self._recent_action_signatures(accepted_trace, limit=3)
    if len(signatures) >= 2 and signatures[-1] == signatures[-2]:
        return 'identical repeated action detected: %s' % signatures[-1]
    return 'none'

def _alternating_warning(self, accepted_trace: List[Dict[str, object]]) -> str:
    names = self._recent_actions(accepted_trace, limit=6)
    if len(names) < 4:
        return 'none'
    if (
        len(set(names[-4:])) == 2
...
def _planning_state_summary(
    ...
    grounding = self._grounding_summary(current_state, request)
    ...
    summary = {
        ...
        'recent_actions': self._recent_actions(accepted_trace),
        'recent_action_signatures': self._recent_action_signatures(accepted_trace),
        'alternating_warning': self._alternating_warning(accepted_trace),
        'identical_repeat_warning': self._identical_repeat_warning(accepted_trace),
...
def _grounding_summary(self, current_state: Dict[str, object], request: str) -> Dict[str, object]:
    highlighted_objects = []
    object_catalog = []
    ...
    request_focus = self._request_focus(request)
    return {
        'request_focus': request_focus,
        'goal_relevant': {
        ...
        'object_catalog': object_catalog,
        'exact_source_matches': self._exact_matches(
            request_focus.get('source', {}),
            source_candidates,
        ),
        'exact_destination_matches': self._exact_matches(
...
def _grounding_verdict_text(self, current_state: Dict[str, object], request: str) -> str:
    grounding = self._grounding_summary(current_state, request)
    request_focus = grounding['request_focus']
...
def _request_focus(self, request: str) -> Dict[str, object]:
    ...
    for marker in move_markers:
        ...
        if idx != -1:
            ...
            break
    source_text = self._strip_leading_goal_verb(source_text)
    destination_text = self._strip_leading_determiner(destination_text)
    ...
    return {
        ...
        'source': {
            'colors': self._extract_attribute_tokens(
                source_text,
                {'red', 'green', 'blue', 'white', 'black', 'yellow'},
            ),
            'kinds': self._extract_attribute_tokens(
                source_text,
                {'block', 'blocks', 'pyramid', 'pyramids', 'box', 'boxes', 'table'},
            ),
            'sizes': self._extract_attribute_tokens(
                source_text,
...
def _strip_leading_goal_verb(self, text: str) -> str:
    words = text.split()
...
def _strip_leading_determiner(self, text: str) -> str:
    words = text.split()
    while words and words[0] in {'the', 'a', 'an'}:
        words = words[1:]
    return ' '.join(words)

def _extract_attribute_tokens(self, text: str, allowed_tokens) -> List[str]:
    tokens = []
...
def _exact_matches(self, target: Dict[str, object], candidates: List[Dict[str, object]]) -> List[Dict[str, object]]:
...
def _cleanup_release_supported(self,
...
def _ap_formula(self, name: str) -> str:
...
def _world_state_delta_summary(self,
    init_world_state: Dict[str, object],
    accepted_trace: List[Dict[str, object]],
) -> str:
...
def _parse_plan(self, content: str) -> Dict[str, object]:
...
_FSM_METHOD_NAMES = (
    '_init_fsm_planner',
    'get_runtime_planning_config',
    'set_runtime_planning_config',
    '_snapshot_json',
    '_json_or_none',
    '_recent_actions',
    '_action_signature',
    '_recent_action_signatures',
    '_identical_repeat_warning',
    '_alternating_warning',
    '_planning_state_summary',
    '_grounding_summary',
    '_grounding_verdict_text',
    '_request_focus',
    '_strip_leading_goal_verb',
    '_strip_leading_determiner',
    '_extract_attribute_tokens',
    '_exact_matches',
    '_property_monitoring_metadata',
    '_run_agent_loop',
    '_agent_config',
    '_agent_flow_spec',
    '_shared_planning_config_extra',
    '_remember_result_path',
    '_observe_live_ap',
    '_execute_shared_step',
    '_request_shared_plan',
    '_verify_shared_action',
    '_summarize_shared_result',
    '_explain_shared_blocked',
    '_verify_single_step',
    '_planning_system_prompt',
    '_plan_instruction',
    '_property_prompt_section',
    '_build_plan_prompt',
    '_execute_grasper_cleanup',
    '_cleanup_release_supported',
    '_ap_formula',
    '_world_state_delta_summary',
    '_diff_ap_states',
    '_build_initial_ap_state',
    '_build_initial_property_state',
    '_parse_plan',
    '_request_plan',
)


def _bind_fsm_methods(agent: OpenAICompatibleShrdluAgent) -> None:
    for name in _FSM_METHOD_NAMES:
        setattr(agent, name, MethodType(globals()[name], agent))


def FsmOpenAICompatibleShrdluAgent(
    env: SimulatorAPI,
    ...
    """Create an OpenAI-compatible SHRDLU agent with FSM planning functions."""
    ...
    _bind_fsm_methods(agent)
    agent._planning_granularity = _normalize_planning_granularity(planning_granularity)
    agent._violation_policy = _normalize_violation_policy(violation_policy)
    retries = int(max_branch_retries)
    if agent._violation_policy in NONBLOCKING_VIOLATION_POLICIES:
        retries = max(1, retries)
    agent._init_fsm_planner(max_branch_retries=retries)
    return agent
